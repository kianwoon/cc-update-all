...
describe('windsurf-extensions discover()', () => {
  ...
  it('returns null when ~/.windsurf/extensions/ directory does not exist', () => {
...
describe('windsurf-extensions parseExtensions()', () => {
  it('extracts gallery-sourced extensions', () => {
    const mod = freshModule();
    const rawJson = [
      makeGalleryEntry('ms-azuretools.vscode-docker', '2.0.0', false),
      makeGalleryEntry('username.my-ext', '1.5.0', true),
    ];

    const result = mod.parseExtensions('/fake/path', rawJson);

    assert.equal(result.extensions.length, 2);
    assert.equal(result.extensions[0].key, 'ms-azuretools.vscode-docker');
    ...
    assert.equal(result.extensions[1].key, 'username.my-ext');
  ...
  it('excludes vsix-sourced extensions', () => {
    const mod = freshModule();
    const rawJson = [makeGalleryEntry('gallery.ext', '1.0.0', false), makeVsixEntry('some.vsix-ext', '2.0.0')];
