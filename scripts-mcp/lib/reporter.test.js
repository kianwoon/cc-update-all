...
test('reporter.formatText: includes tool names', () => {
  const output = reporter.formatText(sampleData, 2);
  assert.ok(output.includes('cursor'), 'should mention cursor');
  assert.ok(output.includes('cline'), 'should mention cline');
});

test('reporter.formatText: includes summary counts', () => {
  const output = reporter.formatText(sampleData, 2);
...
test('reporter.formatText: shows version arrow for updated servers', () => {
  const output = reporter.formatText(sampleData, 2);
  assert.ok(output.includes('1.2.3 -> 1.3.0'), 'should show version arrow');
});

test('reporter.formatText: shows not npx-based for non-npx servers', () => {
  const output = reporter.formatText(sampleData, 2);
  assert.ok(output.includes('(not npx-based)'), 'should show non-npx message');
});

test('reporter.formatText: shows singular "tool" for count of 1', () => {
  const output = reporter.formatText(sampleData, 1);
  assert.ok(output.includes('1 tool'), 'should use singular');
});

test('reporter.formatText: shows plural "tools" for count > 1', () => {
  const output = reporter.formatText(sampleData, 2);
  assert.ok(output.includes('2 tools'), 'should use plural');
});

test('reporter.formatJson: produces valid JSON', () => {
  const output = reporter.formatJson(sampleData);
...
test('reporter.formatText: handles empty tools', () => {
  const emptyData = { tools: {}, summary: { updated: 0, current: 0, skipped: 0, failed: 0 } };
  const output = reporter.formatText(emptyData, 0);
  assert.ok(output.includes('SUMMARY'), 'should still show summary');
});

test('reporter.formatText: handles check_failed with error message', () => {
  const failData = {
    tools: {
      cursor: {
        status: 'ok',
        configPath: '/fake',
        servers: [{ key: 'bad-pkg', package: 'bad-pkg', status: 'check_failed', error: 'network timeout' }],
      },
    },
    summary: { updated: 0, current: 0, skipped: 0, failed: 1 },
  };
  const output = reporter.formatText(failData, 1);
...
test('reporter.formatText: handles not_found status', () => {
  const notFoundData = {
    tools: {
      cursor: {
        status: 'ok',
        configPath: '/fake',
        servers: [{ key: 'private-pkg', package: '@private/pkg', status: 'not_found' }],
      },
    },
    summary: { updated: 0, current: 0, skipped: 1, failed: 0 },
  };
  const output = reporter.formatText(notFoundData, 1);
  assert.ok(output.includes('(not on npm)'), 'should show not on npm message');
});

test('reporter.formatText: handles skipped_floating status', () => {
  const floatingData = {
    tools: {
      cursor: {
        status: 'ok',
        configPath: '/fake',
        servers: [{ key: 'float-pkg', package: '@scope/pkg', status: 'skipped_floating' }],
      },
    },
    summary: { updated: 0, current: 0, skipped: 1, failed: 0 },
  };
  const output = reporter.formatText(floatingData, 1);
